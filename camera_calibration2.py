# Summary - This code performs camera calibration for single camera
# and stereo camera and triangulates points from 2D to 3D with undistortion code
...
class Camera_Calibration:
    ...
    def detect_checker_cornersv2(self, image_files, verify_corners=False, custom_pts_path=None, increment=0, skip=1):
        # define checkerboard
        CHECKERBOARD = (6, 6)
        criteria = (cv2.TERM_CRITERIA_EPS + cv2.TERM_CRITERIA_MAX_ITER + cv2.TERM_CRITERIA_COUNT, 40, 0.001)
        world_scaling = 40  # 40mm
        ...
        for i in range(0, len(image_files), skip):
            ...
            # ret, corners, meta = cv2.findChessboardCornersSBWithMeta(gray, (6, 6), flags=flags)
            ...
            if ret == True and len(corners)==36:
                ...
                # cv2.waitKey(0)
                if corners[0][0][1] > corners[35][0][1]:
                    corners = corners[::-1]

                # elif np.abs(corners[0][0][0] - corners[5][0][0]) < 60:
                #     print('b')
                #     new_corners = np.zeros((36, 2))
                #     r_corners = corners.reshape((6, 6, 2))
                #
                #     new_corners[:6] = np.flip(r_corners[:, 0], 0)
                #     new_corners[6:12] = np.flip(r_corners[:, 1], 0)
                #     new_corners[12:18] = np.flip(r_corners[:, 2], 0)
                #     new_corners[18:24] = np.flip(r_corners[:, 3], 0)
                #     new_corners[24:30] = np.flip(r_corners[:, 4], 0)
                #     new_corners[30:36] = np.flip(r_corners[:, 5], 0)
                #
                #     corners = new_corners.reshape((36, 1, 2)).astype('float32')
                # Check for 90-degree clockwise rotation
                if np.abs(corners[0][0][1] - corners[5][0][1]) > np.abs(corners[0][0][0] - corners[5][0][0]):
                    if (corners[0][0][0] - corners[35][0][0] > 0):
                        corners = rotate(corners, 3)
                    else:
                        corners = rotate(corners, 1)
                # image2 = cv2.drawChessboardCorners(image2, CHECKERBOARD, corners, True)
                # cv2.imshow('Fixed Chessboard', image2)
                # cv2.waitKey(0)
                    ...
                    if custom_pts_path != None:
                        ...
                        for key_name in img_names:
                            ...
                            if key_name == img_filename:
                                ...
                                points_arr = np.array([points_dict[img_filename]]).reshape(
                                    (36, 1, 2))  # shape: (36,1,2)
        ...
        print(len(threedpoints), len(twodpoints))
    ...
    def check_undistortion(self, cam_matrix, dist, distorted_imgs, path_to_save_undistorted, save_undistorted_img=True):
        ...
        for img_name in distorted_imgs:
            ...
            if save_undistorted_img == True:
                ...
                op_file_name = path_to_save_undistorted + folder_name + img_name[uscore_id - 9:][
                                                                        :-4] + "_undistorted.png"

                cv2.imwrite(op_file_name, im_remapped_0)

            # cv2.imshow('img', im_remapped_0)  # cv2.waitKey(0)

        # cv2.destroyAllWindows()

    def calibrate_one_camera(self, threedpoints, twodpoints, gray, check_undistortion=True, flag=True):

        # print("Calibrating........")
        print(gray.shape[::-1])
        # added flags?
        # cv2.CALIB_NINTRINSIC |cv2.CALIB_FIX_PRINCIPAL_POINT |  cv2.CALIB_ZERO_TANGENT_DIST| cv2.CALIB_USE_LU | cv2.CALIB_FIX_K3
        flags =   cv2.CALIB_USE_LU | cv2.CALIB_FIX_K3
        if (flag):
            ret, matrix, distortion, r_vecs, t_vecs = cv2.calibrateCamera(threedpoints, twodpoints, gray.shape[::-1],
                                                                          None,
                                                                          None, flags=flags)
        else:
            ret, matrix, distortion, r_vecs, t_vecs = cv2.calibrateCamera(threedpoints, twodpoints, gray.shape[::-1],
    ...
    def stereo_calibrate(self, image_folder0, image_folder1, output_file_name='calibration_result.pickle',
                         check_undistortion=False, custom_pts_filename=None):
        print(" ")
        ...
        threedpoints, twodpoints, gray = self.detect_checker_cornersv2(image_files)

        ret, matrix_0, distortion_0, r_vecs_0, t_vecs_0, repr_error_0 = self.calibrate_one_camera(threedpoints,
                                                                                                  twodpoints, gray)
        ...
        if check_undistortion == True:
            self.check_undistortion(cam_matrix=matrix_0, dist=distortion_0, distorted_imgs=image_files,
                                    path_to_save_undistorted=self.cam0_img_folder, save_undistorted_img=True)
        ...
        ret, matrix_1, distortion_1, r_vecs_1, t_vecs_1, repr_error_1 = self.calibrate_one_camera(threedpoints,
                                                                                                  twodpoints, gray)
        ...
        if check_undistortion == True:
            self.check_undistortion(cam_matrix=matrix_1, dist=distortion_1, distorted_imgs=image_files,
                                    path_to_save_undistorted=self.cam1_img_folder, save_undistorted_img=True)
        ...
        # image_files_0 = [cv2.imread("/Users/Kushal/Coding/PycharmProjects/UGSRPenguin/Datasets2/"+image_folder0+"/"+file) for file in os.listdir("/Users/Kushal/Coding/PycharmProjects/UGSRPenguin/Datasets2/"+image_folder0)]
        ...
        if (len(image_files_0) > len(image_files_1)):
            image_files_0=image_folder0[0:len(image_files_1)]
        if (len(image_files_0) < len(image_files_1)):
            image_files_1=image_folder1[0:len(image_files_0)]
        print(len(image_files_0),len(image_files_1))
        ...
        threedpoints, twodpoints, gray = self.detect_checker_cornersv2(image_files_0, custom_pts_path=custom_pts_path)
        ...
        threedpoints, twodpoints, gray = self.detect_checker_cornersv2(image_files_1, custom_pts_path=custom_pts_path)

        img_points_cam1 = twodpoints

        print("Number of image points in each cam: ", len(img_points_cam0), len(img_points_cam1))
        ...
        ret, CM1, dist0, CM2, dist1, R, T, E, F = cv2.stereoCalibrate(obj_points, img_points_cam0, img_points_cam1,
                                                                      matrix_0, distortion_0, matrix_1, distortion_1,
                                                                      (self.w, self.h), criteria=criteria, flags=flags)
        ...
        # matrix_1 = P2
        # Save output
        calib_result = {'camera_matrix_c0': matrix_0, 'distortion_c0': dist0, 'camera_matrix_c1': matrix_1,
                        'distortion_c1': dist1, 'c0_R_c1': R, 'c0_t_c1': T, 'essential_matrix': E,
                        'fundamental_matrix': F, 'stereo_repr_error': ret, 'cam0_repr_error': repr_error_0,
        ...
        # undistortion
        if check_undistortion == True:
            self.check_undistortion(cam_matrix=matrix_0, dist=dist0, distorted_imgs=image_files_0,
                                    path_to_save_undistorted=image_folder0, save_undistorted_img=True)

            self.check_undistortion(cam_matrix=matrix_1, dist=dist1, distorted_imgs=image_files_1,
                                    path_to_save_undistorted=image_folder1, save_undistorted_img=True)
    ...
    def triangulate_pts(self, projection_mat_0, projection_mat_1, img_pts_0, img_pts_1, frame_0=None, frame_1=None,
                        show_triangulation_result=True, verbose=False):
    ...
    def test_calibration(self):
        calib_cam = Camera_Calibration(cam0_img_folder='20110110-tm1-ub-maca/Calibration/Intrinseque/cam0',
                                       cam1_img_folder='20110110-tm1-ub-maca/Calibration/Intrinseque/cam1')

        calib_cam.stereo_calibrate(image_folder0='20110110-tm1-ub-maca/Calibration/Extrinseque/cam0',
                                   image_folder1='20110110-tm1-ub-maca/Calibration/Extrinseque/cam1',
                                   check_undistortion=False, custom_pts_filename="manual_pts_data.pkl")
        ...
        threedpoints, twodpoints_cam0, gray = calib_cam.detect_checker_cornersv2([cam0_img_files],
                                                                               custom_pts_path="20110110-tm1-ub-maca/Calibration/Extrinseque/cam0/manual_pts_data.pkl")
        threedpoints, twodpoints_cam1, gray = calib_cam.detect_checker_cornersv2([cam1_img_files],
                                                                               custom_pts_path="20110110-tm1-ub-maca/Calibration/Extrinseque/cam1/manual_pts_data.pkl")
        ...
        with open('calibration_result.pickle', 'rb') as file:
            calib_result = pickle.load(file)
        P0, P1 = calib_cam.get_projection_mat(matrix_0=calib_result['camera_matrix_c0'],
                                              matrix_1=calib_result['camera_matrix_c1'], R=calib_result['c0_R_c1'],
                                              T=calib_result['c0_t_c1'])

        p3ds, dist3d_list = calib_cam.triangulate_pts(projection_mat_0=P0, projection_mat_1=P1, img_pts_0=uv_0,
                                                      img_pts_1=uv_1, frame_0=frame_0, frame_1=frame_1,
                                                      show_triangulation_result=True, verbose=True)
        ...
        for pt_id in range(len(dist3d_list)):
            ...
            if dist3d_list[pt_id] > 80:  # removing any distance value that's greater than 8
                print("popped", dist3d_list.pop(pt_id))

        actual_checker_size = 40  # mm
        actualy_size_arr = np.zeros((len(dist3d_list))) + actual_checker_size

        MSE = np.square(np.subtract(actualy_size_arr, dist3d_list)).mean()  # in mm
        RMSE = np.sqrt(MSE)  # in mm
